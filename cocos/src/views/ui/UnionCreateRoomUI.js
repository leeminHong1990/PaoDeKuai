...
"use strict"
var UnionCreateRoomUI = UIBase.extend({
    ctor: function () {
        this._super();
        this.resourceFilename = "res/ui/UnionCreateRoomUI.json";
    },
    ...
    initCreateRoomPanel: function () {
        ...
        //游戏模式
        function game_moede_btn_event(sender, eventType) {
            if (eventType == ccui.Widget.TOUCH_ENDED) {
        ...
        for (var i = 0; i < self.game_mode_list.length; i++) {
            ...
            game_mode_btn.addTouchEventListener(game_moede_btn_event);
        }
        self.game_mode_list[0].setBright(false);
        self.game_mode_list[0].setTouchEnabled(false);
        self.game_mode_panel_list[0].setVisible(true);
    },

    updateSelect: function () {
        var self = this;
        // self.gameMode = 0;       // 游戏模式 0:经典  1:15张  2:癞子
        self.gameRound = 10;     // 房间局数 10局  20局
        ...
        self.gameForce = 0;      // 玩法选择 0:必须管 1:可不要
        self.gameCardnum = 0;    // 牌数选择 0:16张 1:15张 2:一副牌
        self.gamePlays = [0, 1, 0, 0]; // 最后选择 红桃10扎鸟，四带二，四带三   炸弹不可拆
        self.gameEnd = [1, 0, 1, 0];    // 最后选择 三张可少带出完，三张可少带接完，飞机可少带出完   飞机可少带接完
        self.anticheating = 1;   // 防作弊选择 0:关 1:开
    ...
    updateCreateRoom: function () {
        ...
        function create_btn_event(sender, eventType) {
            if (eventType == ccui.Widget.TOUCH_ENDED) {
                ...
                h1global.curUIMgr.unionhall_ui.create_team_room(room_op);
                self.hide();
            }
        }
