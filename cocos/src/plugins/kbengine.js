...
KBEngine.UINT64 = function (lo, hi) {
    ...
    this.toString = function (scale) {
        // fixed by LZR
        // 转10进制过于麻烦，实际上也不需要精确的十进制表示，所以只支持16进制转换
        var low = this.lo.toString(16);
        var high = this.hi.toString(16);
    ...
    this.add = function (num) {
        // 加一个UINT32
        // cc.log("UINT64:add:", this.lo, num)
        this.lo = this.lo + num;
        if (this.lo > 4294967295) {
            this.hi = this.hi + Math.floor(this.lo / 4294967296);
            this.lo = this.lo % 4294967296;
        }
        // cc.log(this.lo)
        return this;
    }
    ...
    this.sub = function (num) {
        // 减一个UINT32
        // cc.log("UINT64:sub:", this.lo, num)
        if (this.lo >= num) {
            this.lo = this.lo - num;
        } else {
            if (this.hi > 0) {
                this.hi = this.hi - 1;
                this.lo = 4294967296 - num + this.lo;
            } else {
                this.hi = 0;
                this.lo = 0;
            }
...
KBEngine.KBEngineApp = function (kbengineArgs) {
    ...
    this.resetSocket = function () {
        try {
            if (KBEngine.app.socket != undefined && KBEngine.app.socket != null) {
                ...
                if (cc.sys.isObjectValid(sock)) {
                    sock.close();
                } else {
                    cc.log("@@@@@@ reset socket is not valid anymore @@@@@");
                }
            }
        }
        catch (e) {
    ...
    this.onerror_before_onopen = function (evt) {
        ...
        KBEngine.app.resetSocket();
        KBEngine.Event.fire("onConnectionState", false);
        h1global.globalUIMgr.info_ui.show_by_info("连接失败，请检查网络！", cc.size(600, 400), function () {
    ...
    this.onerror_after_onopen = function (evt) {
        ...
        h1global.globalUIMgr.info_ui.show_by_info("连接错误，请重新登录游戏！", cc.size(600, 400), function () {
            if ((cc.sys.os == cc.sys.OS_ANDROID && cc.sys.isNative) || (cc.sys.os == cc.sys.OS_IOS && cc.sys.isNative)) {
                h1global.runScene(new LoginScene());
            } else {
                window.location.href = switches.h5entrylink;
    ...
    this.onclose = function () {
        ...
        //if(KBEngine.app.currserver != "loginapp")
        //	KBEngine.app.reset();
        if (cc.director.getRunningScene().className === "GameHallScene" || cc.director.getRunningScene().className === "GameRoomScene" || cc.director.getRunningScene().className === "ArenaScene") {
            ...
            if (info_json) {
                cc.log('[onclose] 尝试重新登录');
                var info_dict = eval('(' + info_json + ')');
                if ((cc.sys.os === cc.sys.OS_ANDROID && cc.sys.isNative) || (cc.sys.os === cc.sys.OS_IOS && cc.sys.isNative)) {
                    var unionid = info_dict["unionid"];
                    KBEngine.app.login("wx_" + unionid, info_dict["password"], JSON.stringify({
                        "userId": info_dict["user_id"],
                        "sex": info_dict["sex"]
                    }));
                } else {
                    var unionid = info_dict["unionid"];
            ...
            // h1global.globalUIMgr.info_ui.show_by_info("连接超时，请重新登录游戏！", cc.size(600, 400), function(){if((cc.sys.os == cc.sys.OS_ANDROID && cc.sys.isNative) || (cc.sys.os == cc.sys.OS_IOS && cc.sys.isNative)){h1global.runScene(new LoginScene());}else{window.location.href = switches.h5entrylink;}})
        }
    }
    ...
    this.close = function () {
        ...
        KBEngine.app.reset();
        h1global.globalUIMgr.info_ui.show_by_info("连接已断开，请重新登录游戏！", cc.size(600, 400), function () {
            if ((cc.sys.os == cc.sys.OS_ANDROID && cc.sys.isNative) || (cc.sys.os == cc.sys.OS_IOS && cc.sys.isNative)) {
                h1global.runScene(new LoginScene());
            } else {
                window.location.href = switches.h5entrylink;
    ...
    this.update = function () {
        ...
        if ((dateObject.getTime() - KBEngine.app.lastTickTime) / 1000 > 15) {
            ...
            if (KBEngine.app.lastTickCBTime < KBEngine.app.lastTickTime) {
                ...
                KBEngine.app.socket.close();
                h1global.globalUIMgr.info_ui.show_by_info("连接超时，请重新登录游戏！", cc.size(600, 400), function () {
                    if ((cc.sys.os == cc.sys.OS_ANDROID && cc.sys.isNative) || (cc.sys.os == cc.sys.OS_IOS && cc.sys.isNative)) {
                        h1global.runScene(new LoginScene());
                    } else {
                        window.location.href = switches.h5entrylink;
    ...
    this.Client_onAppActiveTickCB = function () {
        var dateObject = new Date();
        KBEngine.app.lastTickCBTime = dateObject.getTime();
        // cc.log("Client_onAppActiveTickCB!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    }
    ...
    this.Client_onLoginFailed = function (args) {
        ...
        cc.log("KBEngineApp::Client_onLoginFailed: failedcode(" + KBEngine.app.serverErrs[failedcode].name + "), datas(" + KBEngine.app.serverdatas.length + "):" + KBEngine.app.serverdatas + "!");
        h1global.globalUIMgr.info_ui.show_by_info("连接失败，请重新登录游戏！", cc.size(600, 400), function () {
            if ((cc.sys.os == cc.sys.OS_ANDROID && cc.sys.isNative) || (cc.sys.os == cc.sys.OS_IOS && cc.sys.isNative)) {
                h1global.runScene(new LoginScene());
            } else {
                window.location.href = switches.h5entrylink;
    ...
    this.Client_onCreatedProxies = function (rndUUID, eid, entityType) {
        ...
        if (entity == undefined) {
            var runclass = KBEngine.app.getentityclass("Player" + entityType);
            if (runclass == undefined)
                return;
    ...
    this.Client_onKicked = function (failedcode) {
        ...
        h1global.globalUIMgr.info_ui.show_by_info("与服务器断开连接，请稍后重新登录游戏！", cc.size(600, 400), function () {
            if ((cc.sys.os == cc.sys.OS_ANDROID && cc.sys.isNative) || (cc.sys.os == cc.sys.OS_IOS && cc.sys.isNative)) {
                h1global.runScene(new LoginScene());
            } else {
                window.location.href = switches.h5entrylink;
...
KBEngine.destroy = function () {
    ...
    // h1global.globalUIMgr.info_ui.show_by_info("连接失败，请重新登录游戏！", cc.size(600, 400), function(){if((cc.sys.os == cc.sys.OS_ANDROID && cc.sys.isNative) || (cc.sys.os == cc.sys.OS_IOS && cc.sys.isNative)){h1global.runScene(new LoginScene());}else{window.location.href = switches.h5entrylink;}})
}
