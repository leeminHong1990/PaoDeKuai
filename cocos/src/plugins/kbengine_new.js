...
KBEngine.KBEngineApp = function(kbengineArgs)
{
	...
	this.post_selectserver = function(selectIndex)
	{  
		var bundle = new KBEngine.Bundle();
		bundle.newMessage(KBEngine.messages.Loginapp_selectServer);		
		bundle.writeInt32(selectIndex);
		bundle.writeString(KBEngine.app.username);
		bundle.send(KBEngine.app);
	}

	this.post_userdserver = function(usedserver)
	{  
		var bundle = new KBEngine.Bundle();
		bundle.newMessage(KBEngine.messages.Loginapp_insertCharacterInServer);	
		bundle.writeString(KBEngine.app.username);	
		bundle.writeString(usedserver);
		bundle.send(KBEngine.app);
	}

	this.reloadServerList = function()
	{  
		var bundle = new KBEngine.Bundle();
		bundle.newMessage(KBEngine.messages.Loginapp_reloadServerList);	
		bundle.send(KBEngine.app);
	}
	...
	*/
	...
	this.Client_onPushServerList = function(args)
	{
		var curserver = args.readUint32();
		var usedserver = args.readString();
		var num = args.readUint32();
		var serverlist = [];
		for (var i = 0; i < num; ++i){
			var id = args.readUint32();
			var address = args.readString();
			var port = args.readUint32();
			var time = args.readUint64();
			var name = KBEngine.utf8ArrayToString(args.readBlob());
			serverlist.push([id, name, address, port, time])
		}	
	}
	...
	this.Client_onKicked = function(failedcode)
	{
		...
		s2global.globalUIMgr.reconnect_ui.show();
	}
...
KBEngine.create = function(kbengineArgs)
{
	...
	if(KBEngine.index == undefined){
		new KBEngine.KBEngineIndexApp(kbengineArgs);
		KBEngine.index.reset();
		KBEngine.index.installEvents();
	}
}
...
KBEngine.destroy = function()
{
	...
	if(KBEngine.index == undefined)
	{
		KBEngine.index.uninstallEvents();
		KBEngine.index.reset();
		KBEngine.index = undefined;
	}
}
