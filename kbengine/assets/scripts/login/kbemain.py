...
def onRequestLogin(loginName, password, clientType, datas):
	...
	if switch.DEBUG_BASE == 0 and 'bot_' != loginName[:4]:
		sign_data = loginName + '_' + switch.ACCOUNT_LOGIN_SECRET
		m = hashlib.md5()
		m.update(sign_data.encode())
		if password != m.hexdigest():
			errorno = KBEngine.SERVER_ERR_PASSWORD
		data = json.loads(datas.decode("utf-8"))
		if data.get("version", "") != switch.CLIENT_VERSION:
			errorno = KBEngine.SERVER_ERR_PASSWORD
...
def onRequestCreateAccount(accountName, password, datas):
	...
	if switch.DEBUG_BASE == 0 and 'bot_' != accountName[:4]:
		sign_data = accountName + '_' + switch.ACCOUNT_LOGIN_SECRET
		m = hashlib.md5()
		m.update(sign_data.encode())
		if password != m.hexdigest():
			errorno = KBEngine.SERVER_ERR_PASSWORD
